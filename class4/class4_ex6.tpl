...
Start
  ^BGP router identifier\s+${BGP_RID}.*local AS number\s+${LOCAL_AS}\s*$$ -> BgpOutput

BgpOutput
  ^Neighbor.*State/PfxRcd\s*$$ -> BGPSummary


BGPSummary
  ^${NEIGHBOR}\s+4\s+${REMOTE_AS}\s+\d+\s+\d+\s+\d+\s+\d+\s+\d+\s+${UP_DOWN}\s+${PREFIX_RCVD}\s*$$ -> Record
